...
namespace Ryntra::Compiler {
    ...
    llvm::Value *IRGenerator::getAddress(std::shared_ptr<IASTNode> node) {
        if (auto idNode = std::dynamic_pointer_cast<IdentifierNode>(node)) {
            ...
            return nullptr;
        } else if (auto arrayAccess = std::dynamic_pointer_cast<ArrayAccessNode>(node)) {
            llvm::Value *baseAddr = getAddress(arrayAccess->getArray());
            if (!baseAddr) return nullptr;

            llvm::Value *indexVal = evaluate(arrayAccess->getIndex());
            if (!indexVal) return nullptr;

            if (indexVal->getType()->getIntegerBitWidth() < 64) {
                indexVal = builder->CreateSExt(indexVal, llvm::Type::getInt64Ty(*context), "idxprom");
            }

            std::vector<llvm::Value *> indices;
            indices.push_back(llvm::ConstantInt::get(llvm::Type::getInt64Ty(*context), 0));
            indices.push_back(indexVal);

            llvm::Type *sourceType = nullptr;
            if (auto *allocaInst = llvm::dyn_cast<llvm::AllocaInst>(baseAddr)) {
                sourceType = allocaInst->getAllocatedType();
            } else if (auto *gepInst = llvm::dyn_cast<llvm::GetElementPtrInst>(baseAddr)) {
                sourceType = gepInst->getResultElementType();
            }

            if (!sourceType) return nullptr;

            return builder->CreateInBoundsGEP(sourceType, baseAddr, indices, "arrayidx");
        }
        return nullptr;
    }

    void IRGenerator::visitArrayAccess(std::shared_ptr<ArrayAccessNode> node) {
        llvm::Value *addr = getAddress(node);
        if (!addr) {
            lastValue = nullptr;
            return;
        }
        ...
        if (elemType) {
            lastValue = builder->CreateLoad(elemType, addr, "arrayval");
        } else {
            lastValue = nullptr;
        }
    }
